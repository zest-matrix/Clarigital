...
def checks(path, h):
    ...
    # ---- C10 (S96): markdown that leaked into HTML ------------------------
    # Renders as literal asterisks. Found on 5 pages across expansion batches
    # 1-5 -- mine, from writing emphasis in markdown habit while composing
    # HTML. A *|MERGE|* tag is legitimate and is excluded.
    # Does NOT cover: underscores, backticks, or markdown link syntax.
    _vis = re.sub(r'<script.*?</script>|<style.*?</style>|<pre>.*?</pre>', '', h, flags=re.S)
    _vis = re.sub(r'<[^>]+>', ' ', _vis)
    _md = re.findall(r'\*\*[^*\n]{2,60}\*\*|(?<![\w*|])\*[^*\n|]{2,60}\*(?![\w*|])', _vis)
    yield (WARN, 'C10 no markdown in rendered text', not _md,
           ', '.join(x[:30] for x in _md[:3]))
    ...
    if hs:
        ...
        # S94: was `function NAME(` only. Every AI Atlas page defines its
        # handlers by ASSIGNMENT -- `window.copyPrompt = function` / `const
        # copyPrompt = (` -- so I3 raised a BLOCK on 139 correct pages while
        # audit.py's handler_check, which recognises both forms, stayed green.
        # A BLOCK-level false positive in the gate that runs after every build.
        defined = set(re.findall(r'function\s+(\w+)\s*\(', h))
        defined |= set(re.findall(r'(?:window\.|var\s+|let\s+|const\s+)?(\w+)\s*=\s*(?:async\s+)?(?:function\b|\()', h))
        defined |= set(re.findall(r'(\w+)\s*:\s*(?:async\s+)?function\b', h))
        called = set()
        for a in hs:
