...
# --- S94: derived indexes must be rebuilt AFTER content, not before -------
# QA #7 found /codex/all-guides/ showing one guide's title on three others,
# because S76 fixed the source pages and nothing re-ran the index. QA #8 then
# found the SAME class again -- committed by me in S89, which regenerated
# all-guides and THEN retitled /codex/paid-advertising/.
#
# Running them here makes the order structural instead of remembered.
import subprocess as _sp, sys as _sys, os as _os
for _g in ('gen_all_guides.py', 'gen_counts.py'):
    _p = _g if _os.path.exists(_g) else _os.path.join('/tmp', _g)
    if _os.path.exists(_p):
        _r = _sp.run([_sys.executable, _p], capture_output=True, text=True)
        print('  [%s] %s' % (_g, (_r.stdout.strip().split(chr(10))[-1] if _r.returncode == 0 else 'FAILED')))
    else:
        print('  [%s] not found — derived index NOT rebuilt' % _g)
